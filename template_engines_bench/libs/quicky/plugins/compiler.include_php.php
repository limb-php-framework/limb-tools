...
function quicky_compiler_include_php($params,&$compiler)
{
 ...
 if (!isset($params['file'])) {return $compiler->_syntax_error('include_php: missing \'file\' parameter in include function');}
 if (!isset($params['once'])) {$params['once'] = TRUE;}
 else {$params['once'] = !preg_match('~^0|FALSE$~i',$params['once']);}
